...
def topk_accuracy_multiple_timesteps(preds, labels, ks=(1, 5), selected_class=None):
    accs = np.array(list(
        zip(*[topk_accuracy(preds[:, t, :], labels, ks, selected_class) for t in range(preds.shape[1])])))
    return accs
...
def tta(scores, labels, time_stamps):
